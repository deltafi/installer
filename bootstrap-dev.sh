...
install_linux_packages() {
  ...
  if tool_exists yum; then
    ...
    ${SUDO} yum install -y git curl wget vim skopeo java-17-openjdk java-17-openjdk-devel python3 python3-pip
...
EOF
    ...
    ${SUDO} apt-get update
    ${SUDO} apt-get install -y uidmap dbus-user-session fuse-overlayfs slirp4netns git wget snapd vim skopeo openjdk-17-jdk python3 python3-pip
    if ! tool_exists kubectl; then
    ...
    ${SUDO} apk add ncurses curl git wget vim skopeo python3 py3-pip openjdk17-jdk kubectx helm yq
